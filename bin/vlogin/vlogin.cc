...
public:
	...
	explicit LoginUI(ogSurface &s, int sw, int sh)
	    : surf_(s), sw_(sw), sh_(sh)
	{
	...
	void draw(const std::string &user, const std::string &pass,
	          bool in_pass, const std::string &err)
	{
		int fw = (int)font_.GetWidth();
		int fh = (int)font_.GetHeight();
		...
		/* Panel */
		surf_.ogFillRect(px_ - 4, py_ - 4,
		                 px_ + pw_ + 4, py_ + ph_ + 4, BOX_COLOR);

		/* Accent bar at top of panel */
		surf_.ogFillRect(px_ - 4, py_ - 4,
		                 px_ + pw_ + 4, py_ - 4 + fh, ACCENT_COLOR);
		...
		if (!in_pass)
			surf_.ogFillRect(field_x_ + (int)user.size() * fw, user_y_,
			                 field_x_ + (int)user.size() * fw + fw - 1,
			                 user_y_ + fh - 1, 0x00C0C0C0u);
		...
		if (in_pass)
			surf_.ogFillRect(field_x_ + (int)pass.size() * fw, pass_y_,
			                 field_x_ + (int)pass.size() * fw + fw - 1,
			                 pass_y_ + fh - 1, 0x00C0C0C0u);
...
do_auth(const std::string &reply_mbox,
        ...
        const std::string &password)
{
	...
	for (int i = 0; i < 2000; i++) {
		if (mpi_fetchMessage(reply_mbox.c_str(), &rmsg) == 0 &&
...
static void
run_session(const struct auth_response &resp)
{
	static const char *taskbar_argv[] = { "taskbar", nullptr };
	char home_env[140], user_env[48], shell_env[140], path_env[64];

	::snprintf(home_env,  sizeof(home_env),  "HOME=%s",
	...
	static const char *taskbar_envp[] = {
	    home_env, user_env, shell_env, path_env,
	    "TERM=xterm", "LD_LIBRARY_PATH=/lib:/usr/lib", nullptr
	};
	...
	if (pid == 0) {
		...
		::setgid(resp.gid);
		::execve(TASKBAR_PATH,
		         (char * const *)taskbar_argv,
		         (char * const *)taskbar_envp);
		::_exit(1);
	}
	if (pid < 0)
		return;
	...
	int scr_w = 800, scr_h = 600;
	{
		...
		for (int i = 0; i < 5000; i++) {
			if (mbox.try_fetch(reply) &&
			    reply.header == DISPLAY_INFO) {
				struct display_info *di = (struct display_info *)reply.data;
				if (di->screen_w > 0) scr_w = (int)di->screen_w;
				if (di->screen_h > 0) scr_h = (int)di->screen_h;
				got = 1;
				break;
			}
			ubix::yield();
