# This needs to be before ubix.init.mk
.if defined(UBIX_MK_COMPAT_FILE)
.include <${UBIX_MK_COMPAT_FILE}>
.endif
...
WEAKALIASOVERRIDEPASS?=${UBIXSRCDIR}/minix/llvm/bin/weak-alias-module-override.so
GOLD_PLUGIN?=${UBIXSRCDIR}/minix/llvm/bin/LLVMgold.so
...
#
# NEED_OWN_INSTALL_TARGET is set to "no" by pkgsrc/mk/ubix.pkg.mk to
# ensure that things defined by <ubix.own.mk> (default targets,
# INSTALL_FILE, etc.) are not conflicting with ubix.pkg.mk.
#
...
.endif
.else
PRINTOBJDIR=	echo /error/ubix.own.mk/PRINTOBJDIR # avoid infinite recursion
.endif

#
# Determine if running in the NetUBIX source tree by checking for the
...
#
# If _SRC_TOP_ != "", we're within the NetUBIX source tree.
# * Set defaults for UBIXSRCDIR and _SRC_TOP_OBJ_.
# * Define _UBIX_VERSION_DEPENDS.  Targets that depend on the
#   NetUBIX version, or on variables defined at build time, can
#   declare a dependency on ${_UBIX_VERSION_DEPENDS}.
#
.if (${_SRC_TOP_} != "")		# {

UBIXSRCDIR?=	${_SRC_TOP_}
...
_UBIX_VERSION_DEPENDS=	${_SRC_TOP_OBJ_}/params
_UBIX_VERSION_DEPENDS+=	${UBIXSRCDIR}/sys/sys/param.h
_UBIX_VERSION_DEPENDS+=	${UBIXSRCDIR}/sys/conf/newvers.sh
_UBIX_VERSION_DEPENDS+=	${UBIXSRCDIR}/sys/conf/osrelease.sh
...
#
# This is the prefix used for the NetUBIX-sourced tools.
#
_TOOL_PREFIX?=	nb
...
#
# In order to identify NetUBIX to GNU packages, we sometimes need
# an "elf" tag for historically a.out platforms.
#
.if (!empty(MACHINE_ARCH:Mearm*))
MACHINE_GNU_PLATFORM?=${MACHINE_GNU_ARCH}--netubixelf-${MACHINE_ARCH:C/eb//:C/v[4-7]//:S/earm/eabi/}
.elif (${MACHINE_GNU_ARCH} == "arm" || \
     ...
     ${MACHINE_ARCH} == "sparc" || \
     ${MACHINE_ARCH} == "vax")
MACHINE_GNU_PLATFORM?=${MACHINE_GNU_ARCH}--netubixelf
.else
...
.if (${_SRC_TOP_} != "")	# only set INSTPRIV if inside ${UBIXSRCDIR}
.if ${MKUNPRIVED} != "no"
INSTPRIV.unpriv=-U -M ${METALOG} -D ${DESTDIR} -h sha256
.else
INSTPRIV.unpriv=
.endif
INSTPRIV?=	${INSTPRIV.unpriv} -N ${UBIXSRCDIR}/etc
.endif
STRIPFLAG?=	
...
.if exists(${UBIXSRCDIR}/../xsrc)
X11SRCDIR!=		cd "${UBIXSRCDIR}/../xsrc" && pwd
.else
X11SRCDIR=		/usr/xsrc
.endif
...
.if exists(${UBIXSRCDIR}/../extsrc)
EXTSRCSRCDIR!=		cd "${UBIXSRCDIR}/../extsrc" && pwd
.else
