...
// Raw BMP on-disk structs (packed, little-endian)
struct Win3xBitmapHeader {
	...
	Win3xBitmapHeader() :
		ImageFileType(0), FileSize(0),
		Reserved1(0), Reserved2(0), ImageDataOffset(0) {}
	bool IsMatch() const {
		return (ImageFileType == 0x4D42 && FileSize != 0 &&
		        Reserved1 == 0 && Reserved2 == 0 && ImageDataOffset != 0);
	}
};

struct Win3xBitmapInfoHeader {
	uInt32 HeaderSize;
	uInt32 ImageWidth;
	uInt32 ImageHeight;
	...
	Win3xBitmapInfoHeader() :
		HeaderSize(0), ImageWidth(0), ImageHeight(0),
		NumberOfImagePlanes(0), BitsPerPixel(0), CompressionMethod(0),
		SizeOfBitmap(0), HorzResolution(0), VertResolution(0),
		NumColoursUsed(0), NumSignificantColours(0) {}
	bool IsMatch() const {
		return (HeaderSize != 0 && ImageWidth != 0 && ImageHeight != 0 &&
		        CompressionMethod == 0 &&
...
public:
	...
	bool Save(const char *filename, ogSurface &surface,
	          ogImageType imageType, ogImageOptions *options = 0);
private:
