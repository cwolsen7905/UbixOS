 ...
 *
 * lwIP netif bridge for the x86_64 virtio-net driver + the x86_64 network bring-up
 * (x86_64_net_init).
 *
 * The i386 net_init (sys/net/net/init.c) is hard-wired to the e1000/ne2k PCI
 * drivers, so x86_64 has its own here (a sibling of aarch64's net/virtio_netif.c):
 * it brings lwIP up over the polling virtio-net driver (arch/x86_64/dev/virtio_net.c).
 * TX flattens an lwIP pbuf chain and hands it to virtio_net_send(); an RX kernel
 * thread polls the receive used-ring via virtio_net_poll_rx() and feeds each frame
 * to lwIP.
 */

#include <sys/types.h>
#include <string.h>
#include <ubixos/sched.h> /* sched_wait_event_timeout */
#include <sys/klog.h>
...
/* x86_64 virtio-net driver (arch/x86_64/dev/virtio_net.c). */
extern u_int8_t virtio_net_mac[6];
...
/**
 * RX delivery callback (invoked by virtio_net_poll_rx per received frame): wrap the
 * frame in a pbuf and push it into lwIP via netif->input (tcpip_input — thread-safe,
 * posts to the tcpip mailbox).
 */
...
/* Never-true sleep predicate: the RX thread has no RX interrupt to wake it, so it
 * just sleeps out the timeout each idle pass. */
...
/**
 * RX poll thread: drain the virtio receive ring and feed lwIP.  While frames arrive
 * it loops at full speed; on an empty sweep it sleeps ~1 tick (leaving the run
 * queue) instead of busy-yielding, so an idle link costs ~0% CPU.  First-packet
 * latency after idle is one timer tick (10 ms) — fine for a desktop (no RX IRQ yet).
 */
...
/** netif status callback: log the DHCP-bound address. */
...
/**
 * Bring up the x86_64 network stack: start lwIP's tcpip thread (which runs the
