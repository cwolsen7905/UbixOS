...
| 3 | Route remaining phys→ptr through the physmap (unwind identity) | **DONE + committed 2026-06-27** (`67729e12f` last). ALL kernel phys-as-pointer access routes through the TTBR1 physmap: pmap table walks, kmalloc, page bitmap, every MMIO register base, all 5 virtio drivers' DMA. |
| 4 | TTBR0 user-only; drop kernel-identity copy; **clang runs** | **DONE + verified 2026-06-27.** `pmap_create_user_space` now `memset`s an EMPTY L1 (no kernel-identity copy) + `USER_L1_MIN=0`. Exposed + fixed 3 identity stragglers Phase 3 missed (each only reachable once TTBR0 is empty): file-backed **mmap** read-destination + its `md_sync_icache` calls used the raw physical frame (syscall.c → `AARCH64_VIRT_OF`); **`aarch64_fork`** passed the raw `TTBR0_EL1` physical root to `pmap_fork_copy` (proc.c → `AARCH64_VIRT_OF`); **`elf64_load_demand`** boundary page `md_sync_icache((uintptr_t)frame)` → `kframe` (MI). Demand paging re-enabled (`execfile.c`: `dr = elf64_load_demand(...)`). **Verified under tcg, 0 exceptions, boots to Login:** dynamic `clang --version` → `clang version 18.1.8` (eager + mmap shared-lib path); **static `llvm-ar` (ET_EXEC, first LOAD @0x200000 — the block-0 collision case) → `LLVM (http://...` via the demand-fault path.** x86_64 kernel still links green (only `elf64_demand.c` is MI). |
| 5 | Cleanup + both arches green | in progress — Phase-3/4 demand-debug kprintfs already gone; both arches build green. Remaining: prune dead bring-up demo calls + stale identity comments. |
