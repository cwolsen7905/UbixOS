...
| U3 | Build the world libs + `bin/` for aarch64 (de-hardcode i386 in `lib/Makefile` etc.) | Userland | 🟡 **In progress** — the shared world build plumbing is now arch-driven (U3.1: `MUSL_ARCH`/`MUSL_LDEMULATION`/`MUSL_LDSO`/`MUSL_LIBGCC_COMPAT`/`ARCH_NOSIMD` centralized in ubix.platform.mk + ubix.musl.vars.mk; i386 byte-identical), `ubix_api` syscall thunks ported (U3.2: `UBIX_NATIVE_THUNK` macro, SVC `mov x8,#(n\|0x8000)`), and **a real bin/ program (`bin/shell`) links to a valid AArch64 ELF64** (U3.3: musl crt1+libc+ubix_api+libgcc).  `lib/ubix/_start.S` is NOT on the musl path (crt1 provides `_start`).  Next (U3.4): full `bmake world` — remaining lib/ subdirs (objgfx, libmd i386 asm, netsurf libs all hardcode `-m elf_i386`) + lib/ubix Makefile |
